...
> **Last refresh**: 2026-06-17 (cyrius 6.1.27 → 6.2.20 + dep pin sweep — sakshi 2.2.10 → 2.3.1, sigil 3.7.8 → 3.8.0, majra 2.4.5 → 2.4.7, bote 2.7.3 → 2.7.6, libro 2.7.2 → 2.7.4; **VERSION 1.2.0 → 1.2.1** + `src/types.cyr` engine string + CHANGELOG 1.2.1 stanza + dist bundles rebuilt at v1.2.1; fixed the `sha256` SIGILL recurrence under 6.2.x — cyrius 6.2.x auto-associates stdlib only for first-party references, so the `thread_local` declaration stopped pulling the module and `sha256` `ud2`'d again; fixed with an explicit `include "lib/thread_local.cyr"` in `src/lib.cyr` + `src/lib_core.cyr` (sigil's documented opt-in); also forced the sigil bump because the 3.7.8 bundle's un-inlined includes (`src/sha_ni.cyr`) don't build under 6.2.x. `CLAUDE.md` + README + `dependency-watch.md` dep blocks refreshed. Prior refresh 2026-06-10 at the 1.2.0 cut.) | **Refresh cadence**: when docs are touched, update the affected row. No release attachment.
