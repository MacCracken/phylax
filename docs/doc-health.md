...
- ✅ ~~`tests/phylax.tcyr` — `sha256 empty` assertion fails~~ — **fixed** in the 1.1.1 cut: rewrote `file_sha256` in `src/analyze.cyr:245` to bypass sigil's `sha256_hex` and compose `sha256()` + phylax's local `hex_encode()` directly. The dispatch-collision root cause (sigil's bundle binds `hex_encode` at bundle-compile time, sidestepping phylax's last-def-wins override at link) was confirmed against the cyrius 5.10.44 toolchain. Three other call sites (`types.cyr:270`, `integration.cyr:340`, `quarantine.cyr:155`) carry the same latent issue but no failing assertions today — folded into the broader duplicate-fn cleanup below.
