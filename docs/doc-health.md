...
> **Last refresh**: 2026-05-11 (initial audit + cyrius 5.7.48 → 5.10.44 + dep pin sweep — sakshi 2.1.0 → 2.2.4, sigil 2.9.5 → 3.1.1, majra 2.4.1 → 2.4.4, bote 2.5.1 → 2.7.1; `lib/` dropped from the tree and gitignored, contract moved to the pin) | **Refresh cadence**: when docs are touched, update the affected row. No release attachment.
...
|---|---|---|
| 🟡 **Stale — refresh in place** | 6 | `CLAUDE.md` (toolchain + dep version block touched in this sweep — Cyrius pin 5.7.34 → 5.10.44, sakshi/sigil/majra/bote versions all bumped; rest of the doc is current); `cyrius.cyml` non-doc but the version block is the source the docs cite; `CHANGELOG.md` (1.1.0 is the latest stanza but the post-1.1.0 toolchain + dep sweep on this date has not been written up yet); `docs/development/dependency-watch.md` (carries the pre-bump pin matrix); `docs/development/roadmap.md` (pre-bump status of the aarch64 chain row); `docs/development/2026-04-28-aarch64-handoff.md` (pre-bump snapshot — see read-through note). |
| 🟠 **Read-through outstanding** | 4 | `README.md` (verify version banner + dep pins block still matches reality after the sweep); `docs/architecture/overview.md` (verify module map still matches `src/lib.cyr` ordering after the sweep — no code moved, but a read-through is owed); `docs/development/threat-model.md` (verify trust-boundary claims still apply against sigil 3.x's PQ surface — phylax uses only SHA-256 hex, so should be a no-op, but a confirmation read-through is owed); `docs/development/issues/2026-04-30-cyrius-stdlib-issues.md` (whether the listed cyrius-stdlib gaps are still open at 5.10.44 — `sys_stat`/`sys_fstat` may have closed transitively; see footer note). |
| ✅ **Fresh — touched in this sweep or current as written** | 3 | `cyrius.cyml` (pins bumped this date; load-bearing source the docs cite — counted here as the post-sweep authoritative state, even though the doc-side rows that *reference* it are flagged stale above); `doc-health.md` (this file, scaffolded 2026-05-11); `VERSION` (`1.1.0` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`). |
...
**Toolchain + dep sweep completed 2026-05-11** (this is the initial audit; CHANGELOG entry pending — see the stale row above):

- ✅ `cyrius.cyml` — Cyrius pin 5.7.48 → 5.10.44; sakshi 2.1.0 → 2.2.4; sigil 2.9.5 → 3.1.1; majra 2.4.1 → 2.4.4; bote 2.5.1 → 2.7.1. All four tags verified as published on the upstream remotes.
...
- ✅ `lib/` removed from the working tree — 63 previously-tracked vendored files. Next consumer run rehydrates from the new pins.
- ✅ `docs/doc-health.md` — this file, scaffolded as the initial audit.

**Carryover items the sweep did not address** (these are the 🟡 / 🟠 rows above; flagged here so the next doc-touch session has a punch list):

- `CHANGELOG.md` — needs a stanza for the post-1.1.0 toolchain + dep sweep + `lib/` gitignore change. The sweep was small enough that it can ship as `1.1.1` or fold into a wider 1.2.x cycle; either way the bumps need to land in the CHANGELOG before the next consumer pulls a tag.
- `docs/development/dependency-watch.md` — rewrite the pin matrix against the post-sweep state (cyrius 5.10.44, sakshi 2.2.4, sigil 3.1.1, majra 2.4.4, bote 2.7.1).
...
- `docs/development/issues/2026-04-30-cyrius-stdlib-issues.md` — confirm whether listed gaps closed at 5.10.44 (`lib/syscalls_x86_64_linux.cyr` in the 5.10.44 stdlib snapshot now defines `sys_stat` / `sys_fstat`, so phylax's `src/syscall_x86_64_linux.cyr` x86 backfill is at minimum redundant — possibly retire-able). A compile error surfaced during the sweep (`undefined variable 'SYS_STAT'` at `src/syscall_x86_64_linux.cyr:19`) is independent of the dep bumps and tracks to stdlib include-ordering on 5.10.x — out-of-scope for this sweep; owed as a follow-up.
...
| `CHANGELOG.md` | 2026-04-30 | 🟡 Stale | Source of truth for shipped work. Last entry is `1.1.0` (2026-04-30). The 2026-05-11 toolchain + dep sweep + `/lib/` gitignore change needs a stanza before the next release tag — see the "Carryover" block above. |
| `CLAUDE.md` | 2026-05-11 | 🟡 Stale (partial — fixed) | Project Identity language version + Dependencies block were bumped in the 2026-05-11 sweep (5.7.34 → 5.10.44; sakshi/sigil/majra/bote pins). Rest of the file is current. Future toolchain bumps touch the same two lines. |
...
| `VERSION` | 2026-04-30 | ✅ Fresh | `1.1.0` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`. Bumps with the next ship. |
