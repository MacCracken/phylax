...
> **Last refresh**: 2026-06-10 (cyrius 5.10.44 → 6.1.25 + dep pin sweep — sakshi 2.2.4 → 2.2.10, sigil 3.1.1 → 3.7.8, majra 2.4.4 → 2.4.5, bote 2.7.1 → 2.7.3, libro 2.6.3 → 2.7.2; **bayan stdlib carve migration** — dropped `json`/`toml`/`csv`/`base64`/`bigint` from `[deps] stdlib`, added `bayan` (+ `slice`, `thread_local`); **VERSION 1.1.1 → 1.2.0** + `src/types.cyr` engine string 1.1.0 → 1.2.0 + CHANGELOG 1.2.0 stanza; fixed a `sha256` SIGILL (undeclared `thread_local` → cyrius-6.1.x `ud2`) and a clean-checkout build break from the carve; **ci.yml + release.yml** now `rm -rf lib && cyrius deps` so a stale `lib/` can't shadow the snapshot; `dependency-watch.md` + README dep block refreshed. Prior refresh 2026-05-11 at the 1.1.1 cut.) | **Refresh cadence**: when docs are touched, update the affected row. No release attachment.
