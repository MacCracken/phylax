...
> **Last refresh**: 2026-06-10 (cyrius 5.10.44 → 6.1.25 + dep pin sweep — sakshi 2.2.4 → 2.2.10, sigil 3.1.1 → 3.7.8, majra 2.4.4 → 2.4.5, bote 2.7.1 → 2.7.3, libro 2.6.3 → 2.7.2; stdlib additions `slice` + `thread_local`; **VERSION bumped 1.1.1 → 1.2.0** + `src/types.cyr` engine string 1.1.0 → 1.2.0 + CHANGELOG 1.2.0 stanza; fixed a `sha256` SIGILL — undeclared `thread_local` resolved to a cyrius-6.1.x `ud2`; `dependency-watch.md` pin matrix + README dep block refreshed; `bayan` dep deferred to 1.2.1 pending cyrius 6.1.27 binary-cap raise. Prior refresh 2026-05-11 at the 1.1.1 cut.) | **Refresh cadence**: when docs are touched, update the affected row. No release attachment.
...
|---|---|---|
| 🟡 **Stale — refresh in place** | 2 | `docs/development/2026-04-28-aarch64-handoff.md` (pre-bump snapshot — see read-through note); `docs/development/issues/2026-04-30-cyrius-stdlib-issues.md` (walk-through owed against the 5.10.44 → 6.1.25 stdlib). `dependency-watch.md` was 🟡 and is now ✅ 1.2.0-fresh. |
...
| ✅ **Fresh — touched in this sweep or current as written** | 8 | `cyrius.cyml` (pins bumped this sweep); `CHANGELOG.md` (1.2.0 stanza landed 2026-06-10); `CLAUDE.md` (toolchain + dep version block matches the 1.2.0 pins); `VERSION` (`1.2.0` — single source of truth); `README.md` (dep block + build stats refreshed at the 1.2.0 cut); `dependency-watch.md` (pin matrix rewritten this sweep); `.github/workflows/ci.yml` (Format check step fixed for `--check` semantics); `doc-health.md` (this file, refreshed at the 1.2.0 cut). |
...
|---|---|---|---|
| `README.md` | 2026-06-10 | ✅ Fresh | Dependency pins block + build stats refreshed at the 1.2.0 cut (cyrius 6.1.25; sakshi 2.2.10 / sigil 3.7.8 / majra 2.4.5 / bote 2.7.3 / libro 2.7.2; ~1.9 MB binary, 177 tests). The stale pre-1.1.1 banner (cyrius 5.1.7 / sigil 2.1.2 / bote 2.5.1) it carried for several cuts is finally corrected. |
| `CHANGELOG.md` | 2026-06-10 | ✅ Fresh | Source of truth for shipped work. `[1.2.0] - 2026-06-10` stanza captures the cyrius 6.1.25 bump, dep pin sweep, `slice` + `thread_local` stdlib additions, the `sha256` SIGILL fix, and the `bayan`/1.2.1 carryover. `[1.1.1]` stanza preserved below it. |
| `CLAUDE.md` | 2026-06-10 | ✅ Fresh | Project Identity language version + Dependencies block refreshed at the 1.2.0 cut (6.1.25; sakshi 2.2.10 / sigil 3.7.8 / majra 2.4.5 / bote 2.7.3 / libro 2.7.2; stdlib list extended with `slice` + `thread_local`). Rest of the file is durable rules. |
...
| `VERSION` | 2026-06-10 | ✅ Fresh | `1.2.0` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`. Bumped 1.1.1 → 1.2.0 at the cyrius 6.1.25 sweep. Note: the engine-reported version lives separately in `src/types.cyr` (`VERSION = "1.2.0"`); it was left at `1.1.0` through 1.1.1 (no-source-change release) and is now re-synced. |
...
| `dependency-watch.md` | 2026-06-10 | ✅ Fresh | Pin matrix rewritten against the 1.2.0 sweep (cyrius 6.1.25 / sakshi 2.2.10 / sigil 3.7.8 / majra 2.4.5 / bote 2.7.3 / libro 2.7.2). Stdlib module list extended to 35 (`slice`, `thread_local` added); `bayan` documented as intentionally-undeclared pending cyrius 6.1.27. |
