...
> **Last refresh**: 2026-08-23 (cyrius 6.4.66 → 6.5.35 + dep pin sweep — sakshi 2.4.6 → 2.4.11, sigil 3.12.1 → 3.12.9, majra 2.5.1 → 2.7.0, bote 3.1.4 → 3.3.7, libro 2.8.2 → 2.8.12; **VERSION 1.2.4 → 1.2.5** + CHANGELOG 1.2.5 stanza + dist bundles rebuilt at v1.2.5. Three structural items: (1) **sakshi + sigil are now FOLDED STDLIB MODULES** — moved out of `[deps.X]` git blocks into `[deps] stdlib`, because a git dep is classified out of the stdlib leaves and both names were consequently missing from the generated consumer sidecars; `dist/phylax.deps` went from **1 leaf to 32**, core from 1 to 15. `[deps.X]` blocks drop 5 → 3. (2) **the CI fmt gate was broken by cyrius 6.5.28** — `cyrius fmt` now rewrites in place and stdout is a one-line report, so the old `diff <(cyrius fmt "$f") "$f"` idiom both reformatted the checkout it audited and failed the whole tree; moved to `cyrius fmt --check`. The same release made the formatter paren-aware → 63 whitespace-only lines across 10 files. (3) **`src/types.cyr` VERSION drift** — it hardcoded `1.2.3` while VERSION said 1.2.4, so the binary and every report's `scanner_version` lagged two releases; fixed and now CI-gated. `CLAUDE.md` + README + `dependency-watch.md` + `architecture/overview.md` dep blocks refreshed to the 1.2.5 pins (overview's table had been stale since the 5.1.7 era). Roadmap backlog reworked: agnosys item closed, JSON/SARIF renderer SIGSEGV and dep-pruning items added — the SIGSEGV item was then FIXED on main immediately after (see the CHANGELOG `[Unreleased]` stanza) and is struck from the backlog. Prior refresh 2026-07-17 at the 1.2.4 cut.) | **Refresh cadence**: when docs are touched, update the affected row. No release attachment.
...
| `CHANGELOG.md` | 2026-08-23 | ✅ Fresh | Source of truth for shipped work. `[1.2.5] - 2026-08-23` stanza captures the cyrius 6.5.35 sweep, the sakshi/sigil fold out of git deps, the broken-CI-fmt-gate and `src/types.cyr` VERSION-drift fixes, and — importantly — a **Performance** section documenting that 6.5.19 changed the bench harness, so rows are NOT comparable across that boundary without subtracting one timer floor. Also carries a **Known, not fixed** entry for the JSON/SARIF renderer SIGSEGV. |
| `CLAUDE.md` | 2026-08-23 | ✅ Fresh | Project Identity language version + Dependencies block refreshed at the 1.2.5 cut (6.5.35; sakshi 2.4.11 / sigil 3.12.9 / majra 2.7.0 / bote 3.3.7 / libro 2.8.12). Dependencies section restructured into **folded stdlib modules** vs **git deps**, with the do-not-re-add warning and the `stdlib`-array comment-parsing trap. Rest of the file is durable rules. |
...
| `VERSION` | 2026-08-23 | ✅ Fresh | `1.2.5` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`. ⚠ It is **not** the only version string: `src/types.cyr` hardcodes the one `--version` prints and reports stamp as `scanner_version`, and it had silently lagged two releases (read 1.2.3 while VERSION said 1.2.4). Now CI-gated against this file. |
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `roadmap.md` | 2026-08-23 | ✅ Fresh | Backlog reworked at the 1.2.5 cut: the transitive-agnosys dep-hygiene item is closed (sigil is long past 3.8.1), and two evidenced items added — the **JSON/SARIF renderer SIGSEGV** (with repro, the untested-renderer gap, and a suspect call site) and **dep pruning**, which now records the measurement that simply deleting the vestigial majra/libro blocks makes things *worse* (pull survives via bote; locked files 65 → 79). |
