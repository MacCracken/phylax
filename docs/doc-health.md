...
> **Last refresh**: 2026-07-17 (cyrius 6.3.15 → 6.4.66 + dep pin sweep — sakshi 2.4.3 → 2.4.6, sigil 3.9.8 → 3.12.1, majra 2.5.0 → 2.5.1, bote 2.7.7 → 3.1.4, libro 2.7.9 → 2.8.2; **VERSION 1.2.3 → 1.2.4** + CHANGELOG 1.2.4 stanza + dist bundles rebuilt at v1.2.4. Two structural items: (1) **bote 2.x → 3.x major** — the 11 hand-picked `src/*.cyr` modules stopped linking (3.0.0's `dispatch.cyr` pulls new `prompts`/`resources` → 5 undefined fns), switched `[deps.bote]` to the upstream `dist/bote-core.cyr` bundle; this drops the last transitive references to majra/libro, so both are now vestigial (staged + DCE-pruned, kept version-tracked). (2) **`ERR_*` → `PHYLAX_ERR_*`** — bare error codes collided with sakshi's `ERR_TIMEOUT` (conflicting-value link warning); namespaced all eleven (mirrors bote `BOTE_ERR_*` / libro `LIBRO_ERR_*`). `CLAUDE.md` + README + `dependency-watch.md` dep blocks refreshed to the 1.2.4 pins (these had lagged since 1.2.1 — 1.2.2/1.2.3 didn't touch them). Prior refresh 2026-06-17 at the 1.2.1 cut.) | **Refresh cadence**: when docs are touched, update the affected row. No release attachment.
...
|---|---|---|---|
| `README.md` | 2026-06-10 | ✅ Fresh | Dependency pins block + build stats refreshed at the 1.2.0 cut (cyrius 6.1.27; sakshi 2.2.10 / sigil 3.7.8 / majra 2.4.5 / bote 2.7.3 / libro 2.7.2; ~1.9 MB binary, 177 tests). The stale pre-1.1.1 banner (cyrius 5.1.7 / sigil 2.1.2 / bote 2.5.1) it carried for several cuts is finally corrected. |
| `CHANGELOG.md` | 2026-06-10 | ✅ Fresh | Source of truth for shipped work. `[1.2.0] - 2026-06-10` stanza captures the cyrius 6.1.27 bump (crossing the 6.1.25 bayan + 6.1.26 ganita carves), dep pin sweep, the bayan-carve `[deps]` reshape (+ `slice`, `thread_local`), and the `sha256` SIGILL fix. `[1.1.1]` stanza preserved below it. |
| `CLAUDE.md` | 2026-06-10 | ✅ Fresh | Project Identity language version + Dependencies block refreshed at the 1.2.0 cut (6.1.27; sakshi 2.2.10 / sigil 3.7.8 / majra 2.4.5 / bote 2.7.3 / libro 2.7.2; stdlib list extended with `slice` + `thread_local`). Rest of the file is durable rules. |
...
| `VERSION` | 2026-06-10 | ✅ Fresh | `1.2.0` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`. Bumped 1.1.1 → 1.2.0 at the cyrius 6.1.27 sweep. Note: the engine-reported version lives separately in `src/types.cyr` (`VERSION = "1.2.0"`); it was left at `1.1.0` through 1.1.1 (no-source-change release) and is now re-synced. |
...
| `dependency-watch.md` | 2026-06-10 | ✅ Fresh | Pin matrix rewritten against the 1.2.0 sweep (cyrius 6.1.27 / sakshi 2.2.10 / sigil 3.7.8 / majra 2.4.5 / bote 2.7.3 / libro 2.7.2). Stdlib module list (31) re-shaped for the bayan carve — `json`/`toml`/`csv`/`base64`/`bigint` dropped, `bayan` + `slice` + `thread_local` added. Upgrade-notes section documents the `rm -rf lib && cyrius deps` rehydration contract. |
