...
- **stdlib**: string, fmt, alloc, vec, str, syscalls, io, args, assert, hashmap, regex, fs, net, tagged, fnptr, callback, thread, bench, bounds, math, process, chrono, freelist, http, mmap, ct, keccak, random, slice, thread_local, **bayan** (cyrius 6.1.25 carve — supplies json/toml/csv/base64/bigint/cyml/u128 via back-compat aliases)
- **sakshi** 2.2.10 — structured logging (`dist/sakshi.cyr` bundle)
- **sigil** 3.7.8 — cryptographic primitives (SHA-256, SHA-NI dispatch)
- **majra** 2.4.5 — pubsub/counter (transitively via bote events_majra)
- **bote** 2.7.3 — MCP tool registry and dispatch
- **libro** 2.7.2 — explicit pin matching bote's transitive libro pin
