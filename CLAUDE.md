...
`tests/phylax-core.tcyr`; 404 assertions total as of the 1.2.6 P(-1) closeout) so a
...
src/<module>.cyr      — 17 modules (types, utils, analyze, strings, script, hashing,
                        ...
                        integration, cli)
tests/test_*.tcyr     — per-module test files (17 files, 404 assertions)
...
| `utils.cyr` | core+full | memmem, hex codec, byte helpers, `in_bounds` offset validator |
...
- **stdlib**: string, fmt, alloc, vec, str, syscalls, io, args, assert, hashmap, regex, fs, net, tagged, fnptr, atomic, sync, thread, bench, bounds, math, process, chrono, freelist, http, mmap, ct, keccak, random, slice, thread_local, **bayan** (cyrius 6.1.25 carve — supplies json/toml/csv/base64/bigint/cyml/u128 via back-compat aliases), **sakshi**, **sigil**

### Folded stdlib modules (NOT git deps)

- **sakshi** 2.4.11 — structured logging. Folded into the toolchain snapshot at the 6.5.18 line.
- **sigil** 3.12.9 — cryptographic primitives (SHA-256, SHA-NI dispatch). Folded; 6.5.35 vendors exactly 3.12.9.

⚠ Both are declared in `[deps] stdlib`, **not** as `[deps.X]` git blocks, and must stay that way.
`cyrius distlib` classifies a git dep out of the stdlib leaves, so a folded module declared as a git
dep is dropped from `dist/phylax.deps` / `dist/phylax-core.deps` and consumers link with `sha256_*` /
`sakshi_*` undefined. A git dep also *overlays* the snapshot, so a lagging pin silently downgrades a
folded module. The `cyrius` pin is the only knob that moves these two.

⚠ Comments inside the `stdlib` array are parsed, not skipped: a `[` in one truncates the array
silently, and a `"quoted phrase"` in one becomes a module name. Keep rationale below the closing `]`.

### Git deps

- **majra** 2.7.0 — pubsub/counter; **vestigial** since bote-core dropped `events_majra` (phylax calls it directly nowhere) — staged, DCE-pruned, kept version-tracked pending a dep-pruning pass
- **bote** 3.3.7 — MCP tool registry and dispatch, consumed via the transport-free **`dist/bote-core.cyr`** bundle (the old 11 hand-picked `src/*.cyr` modules no longer link under bote 3.x)
- **libro** 2.8.12 — **vestigial** belt-and-suspenders pin aligned to bote 3.3.7's transitive libro; not referenced by phylax or bote-core (DCE-pruned). ⚠ It nonetheless pulls sigil as a *git* dep, staging granular `lib/sigil_*.cyr` leaves alongside the folded monolith (both 3.12.9; duplicate defs are inert but cost ~350 KB of binary). Dropping this block does not avoid it — the pull survives via bote — so the fix belongs upstream.
