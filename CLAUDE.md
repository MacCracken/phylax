...
- **stdlib**: string, fmt, alloc, vec, str, syscalls, io, args, assert, hashmap, regex, fs, net, tagged, fnptr, atomic, sync, thread, bench, bounds, math, process, chrono, freelist, http, mmap, ct, keccak, random, slice, thread_local, **bayan** (cyrius 6.1.25 carve — supplies json/toml/csv/base64/bigint/cyml/u128 via back-compat aliases)
- **sakshi** 2.4.6 — structured logging (`dist/sakshi.cyr` bundle)
- **sigil** 3.12.1 — cryptographic primitives (SHA-256, SHA-NI dispatch); imposes cyrius ≥ 6.4.65 (bundle calls `thread_local_alloc`)
- **majra** 2.5.1 — pubsub/counter; **vestigial** since bote-core dropped `events_majra` (phylax calls it directly nowhere) — staged, DCE-pruned, kept version-tracked pending a dep-pruning pass
- **bote** 3.1.4 — MCP tool registry and dispatch, consumed via the transport-free **`dist/bote-core.cyr`** bundle (the old 11 hand-picked `src/*.cyr` modules no longer link under bote 3.x)
- **libro** 2.8.2 — **vestigial** belt-and-suspenders pin aligned to bote 3.1.4's transitive libro; not referenced by phylax or bote-core (DCE-pruned)
